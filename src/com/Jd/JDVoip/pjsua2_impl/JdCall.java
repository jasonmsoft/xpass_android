 ...
 */
public class JdCall extends Call{

    private JdCallObserver   observer;

    public  JdCall(JdAccount jd_Acc, int call_id, JdCallObserver obr)
    {
        super(jd_Acc, call_id);
        observer = obr;
    }
        ...
        for (int i = 0; i < cmiv.size(); i++) {
                                    ...
                                    pjsua_call_media_status.PJSUA_CALL_MEDIA_REMOTE_HOLD))
            {
               ...
               Media m = getMedia(i);
                ...
                // connect ports
                try {
                    JdPjsipApp.getInstance().getEndpoint().audDevManager().getCaptureDevMedia().
                            startTransmit(am);
                    am.startTransmit(JdPjsipApp.getInstance().getEndpoint().audDevManager().
                            getPlaybackDevMedia());
                } catch (Exception e) {
                    continue;
                }
